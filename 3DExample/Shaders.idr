...
enableVertexAttribArray ((MkGLProg p),(MkCxt c)) loc = do
  mkForeign (FFun "%0.enableVertexAttribArray(%1)" [FPtr,FInt] FUnit) c loc
  return ((MkGLProg p),(MkCxt c))

-- Set the array pointer for the GL program so it knows where to find some vertices.
setVertexAttribPointer : Int -> Int -> (GLProgram,GLCxt) -> IO (GLProgram,GLCxt)
setVertexAttribPointer n loc ((MkGLProg p),(MkCxt c)) = do
  mkForeign (FFun
             "%0.vertexAttribPointer(%1, %2, %0.FLOAT, false, 0, 0)"
             [FPtr,FInt,FInt] FUnit
            ) c loc n
  return ((MkGLProg p),(MkCxt c))
